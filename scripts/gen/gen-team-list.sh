...
if [[ ! -d .team/agents ]]; then
  echo "No .team/agents directory - run 'just gen-team-sync' first"
  exit 1
fi
...
if [[ $count -eq 0 ]]; then
  echo "No specialists found - run 'just gen-team-sync'"
  exit 1
else
  ...
  echo ""
  echo "Next: just gen-team-tools to setup tool generation"
fi
...
#   SSH_HOST=provider.akash.example.com SSH_P=32123 \
...
#   SSH_HOST=provider.akash.example.com SSH_P=32456 \
#   SSH_KEY=~/.oline/ssh-key \
#   SNAPSHOT_LOCAL_PATH=/var/oline/cache/morocco-1.tar.lz4 \
...
if [ "$MODE" = "fetch-node" ]; then
    ...
    echo "[snapshot-transfer] Fetching data dir from ${SSH_HOST}:${SSH_P}"
    echo "[snapshot-transfer] Remote: ${REMOTE_DATA_DIR}"
    echo "[snapshot-transfer] Format: ${SNAPSHOT_FORMAT}"
    echo "[snapshot-transfer] → ${SNAPSHOT_LOCAL_PATH}"

    ssh "${SSH_OPTS[@]}" -i "$SSH_KEY" -p "$SSH_P" \
...
if [ "$MODE" = "push-node" ]; then
    ...
    echo "[snapshot-transfer] → ${SSH_HOST}:${SSH_P}:${SNAPSHOT_REMOTE_PATH}"
    ...
    cat "$SNAPSHOT_LOCAL_PATH" | ssh "${SSH_OPTS[@]}" -i "$SSH_KEY" -p "$SSH_P" \

...
//     let mut generated_privkey: Option<ssh_key::PrivateKey> = None;
...
//         tracing::info!("  Generated SSH key (encrypting after password prompt)");
//         // Hold the private key — saved encrypted after password is obtained below.
//         generated_privkey = Some(privkey);
...
//     // Save generated SSH key encrypted now that we have the password
//     if let Some(privkey) = generated_privkey {
//         crate::crypto::save_ssh_key_encrypted(&privkey, &key_path, &password)?;
//         tracing::info!("  SSH key encrypted → {}.enc", key_path.display());
//     }
